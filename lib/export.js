...
exports.forLib = function (LIB) {
			...
			function fetchUrl (url) {
				...
				function attempt () {
					...
					return LIB.Q.denodeify(function (callback) {
						return LIB.REQUEST({
							uri: url,
							headers: options.config.source.server.headers || {}
	...
	//						request.resume();
			...
			function parseHtml (url, html) {
				...
				// Convert all 'component:*' attributes to 'component-*'
				...
				function parseInstanciatedHtml (loaded, window, declarations) {
                    ...
                    // Hide all views that are not sections
// TODO: Optionally hide sections. This is needed when not using chscript.
/*
                    $('[data-component-view]').each(function () {
						var elm = $(this);
						if (!elm.attr("data-component-section")) {
							...
							onDownloaded: function () {
								if (!filesMoved[localPath]) {
									filesMoved[localPath] = LIB.QFS.read(localPath).then(function (data) {
										...
										var hash = LIB.CRYPTO.createHash("sha1").update(data).digest("hex").substring(0, 7);
										path += "-" + hash;
										path += "." + ext;
				...
				return ensureDirectories().then(function () {
					...
					return LIB.Q.all(declarations.resources.map(function (resource) {
						...
						if (
							...
							var url = "http://" + options.config.source.server.host + (options.config.source.server.subPath || "") + resource.attributes.href;
...
*/
			...
			function exportComponents (componentGroupAlias, declarations, rewrites) {
				...
				function generateComponentFormats (alias, component) {
					...
					function generateJSX (component, componentHTML) {
						return LIB.Q.fcall(function () {
							...
							while (m = re.exec(html)) {
								...
								html = html.replace(m[0], m[1] + "{" + LIB.CJSON(attributes) + "}" + m[3]);
							}
					...
					function generateCHscript (component, componentHTML) {
						return LIB.Q.denodeify(function (callback) {


							try {
	                            return CVDOM.html2chscript(componentHTML, {
	                                "controlAttributes": {
					...
					return generateHtml(component, componentHTML).then(function (data) {
						...
						return generateCHscript(component, componentHTML).then(function (data) {

							var targetPath = LIB.PATH.join(basePath, alias + ".chscript.js");

							declarations.components[alias].exportPaths["cjs.chscript"] = targetPath;
			...
			return LIB.Q.all(Object.keys(options.config.pages).map(function (alias) {
				...
				var origin = "http://" + options.config.source.server.host + (options.config.source.server.subPath || "");
					...
					return parseHtml(url, html).then(function (declarations) {
						...
						return exportResources(origin, LIB.PATH.dirname(url), declarations).then(function (rewrites) {
							...
							return exportComponents(alias, declarations, rewrites).then(function (basePath) {
								...
								var data = LIB.CJSON(declarations, null, 4);
								// Make all internal paths relative
								data = data.replace(new RegExp('"' + LIB.PATH.dirname(defintionPath).replace(/\//g, "\\/"), "g"), '"{{__DIRNAME__}}');

								return LIB.QFS.write(defintionPath, data);
							});
						});
					});
				});
				...
				function writeDescriptorFile (pageDefinitions) {
					...
					Object.keys(pageDefinitions).forEach(function (pageAlias) {
						...
						declarations.resources.forEach(function (resource) {
							if (
								...
								descriptor.pages[pageAlias].resources.push({
									...
									"fsPath": (resource.exportPath && ("{{__DIRNAME__}}/" + LIB.PATH.relative(
										targetBaseFsPath,
										resource.exportPath
									))) || null,
						...
						Object.keys(declarations.components).forEach(function (componentAlias) {
							descriptor.pages[pageAlias].components[componentAlias] = {
								"uriHtmlPath": "/" + LIB.PATH.relative(
									targetBaseUriPath,
									declarations.components[componentAlias].exportPath
								),
								"fsHtmlPath": "{{__DIRNAME__}}/" + LIB.PATH.relative(
									targetBaseFsPath,
									declarations.components[componentAlias].exportPath
								)
							};
					...
					var data = LIB.CJSON(descriptor, null, 4);
					// Make all internal paths relative
					data = data.replace(new RegExp('"' + targetBaseFsPath.replace(/\//g, "\\/"), "g"), '"{{__DIRNAME__}}');

					return LIB.QFS.write(
						LIB.PATH.join(targetBaseFsPath, "hoisted.json"),
						data
					).then(function () {
						return descriptor;
					});
				...
				LIB.console.verbose("Page definitions:", LIB.CJSON(pageDefinitions, null, 4));
